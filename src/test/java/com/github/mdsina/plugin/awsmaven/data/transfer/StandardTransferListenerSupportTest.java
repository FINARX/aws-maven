...
package com.github.mdsina.plugin.awsmaven.data.transfer;

import com.github.mdsina.plugin.awsmaven.maven.matchers.Matchers;
import com.github.mdsina.plugin.awsmaven.data.TransferListenerSupport;
...
import static com.github.mdsina.plugin.awsmaven.maven.matchers.Matchers.eq;
...
public class StandardTransferListenerSupportTest {
    ...
    public void fireTransferInitiated() {
        ...
        verify(this.transferListener).transferInitiated(
                Matchers.eq(new TransferEvent(this.wagon, this.resource, TransferEvent.TRANSFER_INITIATED, REQUEST_TYPE)));
    }

    @Test
    public void fireTransferStarted() {
        this.transferListenerSupport.fireTransferStarted(this.resource, REQUEST_TYPE);
        verify(this.transferListener).transferStarted(Matchers.eq(new TransferEvent(this.wagon, this.resource,
    ...
    public void fireTransferProgress() {
        ...
        verify(this.transferListener).transferProgress(
                Matchers.eq(new TransferEvent(this.wagon, this.resource, TransferEvent.TRANSFER_PROGRESS, REQUEST_TYPE)),
    ...
    public void fireTransferCompleted() {
        ...
        verify(this.transferListener).transferCompleted(
                Matchers.eq(new TransferEvent(this.wagon, this.resource, TransferEvent.TRANSFER_COMPLETED, REQUEST_TYPE)));
    }
    ...
    public void fireTransferError() {
        ...
        verify(this.transferListener).transferError(Matchers.eq(new TransferEvent(this.wagon, this.resource, exception,
                REQUEST_TYPE)));
    }
}
